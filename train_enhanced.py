...
- Versioned Output: Saves artifacts to trained_models/{service}/{timestamp}/ to prevent overwriting
...
- Hierarchical Data Loading: enhanced_training_data/service_name/*.csv
"""
...
def setup_logging(log_dir="logs", run_id=None):
    ...
    # Use the shared run_id if provided, else generate new one
...
class PipelineResults:
    ...
    test_samples: int = 0
    model_dir: str = ""  # Added to track where models were saved
...
class AutoscalerPipeline:
    def __init__(self, data_dir="enhanced_training_data", increase_threshold=0.1, seq_len=60, 
    ...
    def load_and_prepare_data(self, service_name):
        ...
        # Modified to look in subdirectories: data_dir/service_name/*.csv
        service_path = self.data_dir / service_name
        
        if not service_path.exists():
            # Fallback to flat directory search if subfolder doesn't exist
            self.logger.warning(f"Subdirectory {service_path} not found. Searching flat directory.")
            csv_files = list(self.data_dir.glob(f"{service_name}_*_lstm_prophet_ready.csv"))
        else:
            csv_files = list(service_path.glob("*.csv"))

        if not csv_files:
            raise ValueError(f"No CSV files found for {service_name} in {self.data_dir}")
        
        self.logger.info(f"Found {len(csv_files)} files. Loading...")
        ...
        self.logger.info(f"Raw Combined Shape: {data.shape}")
    ...
    def train_all(self, service_name, run_id):
        self.logger.info(f"Training: {service_name} (Run ID: {run_id})")
        ...
        res = PipelineResults(run_id, service_name, self.scaler_type, lstm_seq_len=self.seq_len)
...
def main():
    ...
    parser.add_argument("--data-dir", default="enhanced_training_data")
    ...
    # Generate a unique run ID for this execution (used for directories and logs)
    ...
    try:
        ...
        results = pipeline.train_all(args.service, run_id)
        
        # Save results JSON
        results_file = Path(args.log_dir) / f"results_{args.service}_{run_id}.json"
        
        if args.save_models:
            # FIX: Create a timestamped subdirectory for this specific run
            # This ensures we never overwrite the scaler or models from a previous good run
            models_dir = Path("trained_models") / args.service / run_id
            models_dir.mkdir(parents=True, exist_ok=True)
            
            # Record the save location in the results object
            results.model_dir = str(models_dir)
            
            logger.info(f"Saving models to: {models_dir}")
            
            # Save the scaler (CRITICAL: Must match the models trained in this run)
